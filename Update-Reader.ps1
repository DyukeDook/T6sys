...
$CategoryConfigs = @(
  [pscustomobject]@{
    id = "t6sys"
    title = "T6Sys"
    description = "Normal systems and general T-6C study files."
    dataFolder = "Data"
  },
  [pscustomobject]@{
    id = "t6emer"
  ...
  [pscustomobject]@{
    id = "trick-11-248"
    title = "11-248 Trick"
    description = "Takeoff, landing, and 11-248 technique study files."
    dataFolder = "11-248 trick"
  }
)
...
function New-LibraryItem {
  ...
  $searchText = [regex]::Replace(($searchParts -join " "), "\s+", " ").Trim()
  ...
  $id = [regex]::Replace($idSource, "[^a-z0-9]+", "-").Trim("-")
  ...
  $relativePath = "$($Category.dataFolder)/$($File.Name)"
  ...
  return [pscustomobject]@{
    ...
    fileName = $File.Name
    path = $relativePath
    dataFolder = $Category.dataFolder
...
foreach ($category in $CategoryConfigs) {
  $categoryDir = Join-Path $Root $category.dataFolder
  if (-not (Test-Path -LiteralPath $categoryDir -PathType Container)) {
    New-Item -ItemType Directory -Path $categoryDir | Out-Null
  }

  $files = Get-ChildItem -LiteralPath $categoryDir -Filter "*.html" -File | Sort-Object LastWriteTime -Descending
  $items = @()

  foreach ($file in $files) {
    $items += New-LibraryItem -File $file -Category $category -SeenIds $seenIds
  }
...
$manifest = [pscustomobject]@{
  ...
  dataFolders = @($CategoryConfigs | ForEach-Object { $_.dataFolder })
...
Write-Host "Found $($allItems.Count) HTML file(s) total"
foreach ($category in $categories) {
  Write-Host " - $($category.title): $($category.totalFiles) file(s) in $($category.dataFolder)"
}
